...
class Tank(Collider):
    ...
    def _shoot(self):
        ...
        target = None
        if Game.instance() and (
                getattr(Game.instance(), "cheat_tank_name", None) == self.name
                or getattr(Game.instance(), "cheat_tank_name", None) == "Both"
        ):
...
class Game:
    ...
    def __init__(self):
        ...
        self.input_boxes = [pygame.Rect(Settings.WIDTH // 2 - 100, 250, 200, 40),
                            pygame.Rect(Settings.WIDTH // 2 - 100, 320, 200, 40)]
        self.player_inputs = ["", ""]
    ...
    def select_difficulty(self):
        ...
        buttons = [
            ("Easy", pygame.Rect((Settings.WIDTH - button_width) // 2, 200, button_width, button_height)),
            ("Medium", pygame.Rect((Settings.WIDTH - button_width) // 2, 300, button_width, button_height)),
            ("Hard", pygame.Rect((Settings.WIDTH - button_width) // 2, 400, button_width, button_height)),
        ]
        while True:
    ...
    def restart(self):
        ...
        if self.difficulty == 0:
            self.obstacles = [Obstacle(450, 225, 75, 300)]
        elif self.difficulty == 1:
            self.obstacles = [
                Obstacle(300, 150, 75, 450),
    ...
    def show_menu(self):
        ...
        setting_button = pygame.Rect(Settings.WIDTH // 2 - 100, 300, 200, 60)
        difficulty_button = pygame.Rect(Settings.WIDTH // 2 - 100, 400, 200, 60)
        instruction_button = pygame.Rect(Settings.WIDTH // 2 - 100, 500, 200, 60)
        ...
        while True:
            ...
            version_text = version_font.render("Beta Version 5.0", True, (150, 150, 150))
    ...
    def show_settings(self):
        ...
        for idx, key in enumerate(sliders):
            slider_rects[key] = pygame.Rect(300, 100 + idx * 80, 200, 20)
        cheat_button = pygame.Rect(300, 450, 200, 40)
        while True:
            ...
            for idx, (key, (val, min_val, max_val)) in enumerate(sliders.items()):
                ...
                if key == "FireRate":
                    name = "FireRate"
                label = font.render(f"{name}: {int(val)}", True, (0, 0, 0))
                self.screen.blit(label, (100, 100 + idx * 80))
            ...
            self.screen.blit(cheat_label, (cheat_button.x + 60, cheat_button.y + 5))
            back_text = font.render("Back", True, (0, 0, 0))
            back_rect = back_text.get_rect(topleft=(50, 500))
            self.screen.blit(back_text, back_rect)
            start_text = font.render("Start", True, (0, 0, 0))
            start_rect = start_text.get_rect(bottomright=(Settings.WIDTH - 50, Settings.HEIGHT - 30))
    ...
    def show_cheat_menu(self):
        ...
        quit_button = pygame.Rect(center_x, start_y + line_height * 4, button_width, button_height)
    ...
    def show_account_input(self):
        ...
        confirm_buttons = [pygame.Rect(Settings.WIDTH // 2 + 120, 250, 100, 40),
                           pygame.Rect(Settings.WIDTH // 2 + 120, 320, 100, 40)]
        ...
        while not all(self.confirmed):
            ...
            self.screen.blit(label, label.get_rect(center=(Settings.WIDTH // 2, 180)))

            for i in range(2):
                pygame.draw.rect(self.screen, (255, 255, 255), self.input_boxes[i], 2)
                ...
                if event.type == pygame.KEYDOWN:
                    for i in range(2):
                        if active_box[i] and not self.confirmed[i]:
                            ...
                            elif len(self.player_inputs[i]) < 10 and event.unicode.isprintable():
    ...
    def draw(self):
        ...
        if self.winner and not self.is_restarting:
        ...
        if self.winner:
            ...
            pygame.draw.rect(self.screen, (220, 20, 60), bg_rect, border_radius=10)
